"""vier-augen: which tier a shell command falls into.

The one place that decides what a command does, shared by every adapter's
guard and by the behaviour tests, so that they never disagree. It names no
agent and no runtime: a command is Operator tier, or it rewrites published
...
# The two answers, in the skill's own words. None means neither.
OPERATOR = "operator"       # hand it over: it makes something permanent or public
REWRITE = "rewrite"         # rewrites published history: never proposed as a fix
...
def check_git(args, depth=0):
    ...
    if sub == "push":
        if any(a in ("-f", "--force", "--mirror") or a.startswith("--force")
            ...
            return (REWRITE, "force-push rewrites published history")
        return (OPERATOR, "git push publishes")
    if sub == "branch":
        if all(a.split("=", 1)[0] in BRANCH_READ_FLAGS for a in rest if a.startswith("-")) \
                and not [a for a in rest if not a.startswith("-")]:
            return None
        return (OPERATOR, "git branch changes branches")
    if sub == "tag":
        ...
        if listing or not positional and all(
            ...
            return None
        return (OPERATOR, "git tag creates or deletes a tag")
    if sub in GIT_READ_SUBACTIONS and rest and rest[0] in GIT_READ_SUBACTIONS[sub]:
        return None
    if sub in GIT_ASK:
        return (OPERATOR, f"git {sub} is Operator tier")
    ...
    if alias is None:
        return None
    if alias.startswith("!"):
        return (OPERATOR, f"git {sub} is a shell alias")
...
def check_segment(segment):
    ...
    if prog in DELETE_PROGRAMS:
        return (OPERATOR, f"{prog} deletes; approve per path")
    if prog == "find" and "-delete" in args:
        return (OPERATOR, "find -delete deletes")
    if args and (prog, args[0]) in PUBLISH:
        return (OPERATOR, f"{prog} {args[0]} publishes")
    return None
