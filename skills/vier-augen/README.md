...
- `guard.py`: a `PreToolUse` hook that catches what text rules miss - compound
  commands, the clauses of `if`, `for` and `while`, `git -C <dir> push`, git
  aliases, branch and tag changes, `npm publish` and the like. It asks for
  Operator-tier steps and blocks force-push. It needs `python3`, and it needs the skill folder around it: the
